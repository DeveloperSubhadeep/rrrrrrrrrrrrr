...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    #         await query.answer(url=f"https://telegram.me/{temp.U_NAME}?start=sendfiles4_{key}")
            ...
            # Check if SendAll is premium-only
            is_premium_only = await db.sendall_mode_status(client.me.id)
            
            # if is_premium_only or ADMINS and not await db.has_premium_access(query.from_user.id):
            if is_premium_only and query.from_user.id not in ADMINS and not await db.has_premium_access(query.from_user.id):
                # 1. Acknowledge the button click to stop the loading icon.
                await query.answer()
                
                # 2. Send a clear, temporary message explaining the restriction.
                alert_text = (
                    f"👋 **Access Denied, {query.from_user.mention}!**\n\n"
                    "The 'Send All' button is an exclusive feature for our ✨ **Premium Subscribers** ✨.\n\n"
                    "Upgrade your account to instantly download all files at once and enjoy an ad-free experience!\n\n"
                    "👉 Use the `/premium` command to see our affordable plans."
                )
                try:
                    # Send the message as a reply
                    temp_alert_msg = await query.message.reply_text(
                        text=alert_text,
                        quote=True,
                        disable_web_page_preview=True
                    )
                    ...
                    await temp_alert_msg.delete()
                except Exception as e:
                    logger.error(f"Error sending/deleting 'sendall' premium alert: {e}")
                
                return # IMPORTANT: Stop further execution for non-premium users.

            # Original logic for premium users or when the feature is enabled for all
            clicked = query.from_user.id
            ident, key = query.data.split("#")
            settings = await get_settings(query.message.chat.id)
            try:
...
#         # --- Start of Premium Check ---
#         if query.from_user.id not in ADMINS and not await db.has_premium_access(query.from_user.id):
#             # 1. Acknowledge the button click
#             await query.answer()
        
#             # 2. Send a clear, temporary message
#             alert_text = (
#                 f"""<b>🚫 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱, {query.from_user.mention}!</b>

# 👋 ᴛʜᴇ ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ / ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ ꜰᴇᴀᴛᴜʀᴇ ɪꜱ ᴇxᴄʟᴜꜱɪᴠᴇʟʏ ᴀᴠᴀɪʟᴀʙʟᴇ ꜰᴏʀ ᴏᴜʀ ✨ ʜᴇʟʟ ᴋɪɴɢ ᴘʀᴇᴍɪᴜᴍ ᴍᴇᴍʙᴇʀꜱ ✨.

#     💎 ᴜᴘɢʀᴀᴅᴇ ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ ᴛᴏ ᴜɴʟᴏᴄᴋ:
#     • ⚡️ ɪɴꜱᴛᴀɴᴛ ꜱᴛʀᴇᴀᴍɪɴɢ ᴀᴄᴄᴇꜱꜱ  
#     • 🛜 ᴜɴʟɪᴍɪᴛᴇᴅ ᴅᴀɪʟʏ ᴅᴏᴡɴʟᴏᴀᴅꜱ  
#     • 🚫 100% ᴀᴅ-ꜰʀᴇᴇ ᴇxᴘᴇʀɪᴇɴᴄᴇ  

#     💰 ᴠɪᴇᴡ ᴘʟᴀɴꜱ: ᴜꜱᴇ ᴛʜᴇ /ᴘʀᴇᴍɪᴜᴍ ᴏʀ /ᴘʟᴀɴ ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴇxᴘʟᴏʀᴇ ᴏᴜʀ ᴀꜰꜰᴏʀᴅᴀʙʟᴇ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ ᴏᴘᴛɪᴏɴꜱ.

#     ━━━━━━━━━━━━━━━
#     <i>🔥 ᴘᴏᴡᴇʀᴇᴅ ʙʏ: 𝐓𝐡𝐞 𝐇𝐞𝐥𝐥 𝐊𝐢𝐧𝐠</i>"""
#             )
#             try:
#                 temp_alert_msg = await query.message.reply_text(
#                     text=alert_text,
...
#                     ]])
#                 )
#                 await asyncio.sleep(15) # Message stays for 30 seconds
#                 await temp_alert_msg.delete()
#             except Exception as e:
#                 logger.error(f"Error sending/deleting stream premium alert: {e}")
            
#             return # Stop further execution for non-premium users
#         # --- End of Premium Check ---


# REPLACE the existing 'generate_stream_link' block in pmfilter.py

    elif DreamxData.startswith("generate_stream_link"):
        # This block is now simple because the button is only shown to authorized users.
        # No permission check is needed here anymore.
        ...
        try:
            ...
            await query.message.edit_reply_markup(
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("⏳ Generating Link...", callback_data="hiding")
                ]])
            )
            ...
            dreamx_stream = f"{URL}watch/{str(log_msg.id)}/{file_name}?hash={file_hash}"
            dreamx_download = f"{URL}{str(log_msg.id)}/{file_name}?hash={file_hash}"
            ...
            stream_buttons = await query.message.edit_reply_markup(
                reply_markup=InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton("🛜 Download Link 🛜", url=dreamx_download),
            ...
            await stream_buttons.delete()
            return

        except Exception as e:
            print(e)
            await query.answer(f"⚠️ SOMETHING WENT WRONG: {e}", show_alert=True)
            try:
        ...
        #     # Show a "Generating..." message to the user
        #     generating_msg = await query.message.edit_reply_markup(
        #         reply_markup=InlineKeyboardMarkup([[
        #             InlineKeyboardButton("⏳ Generating Link...", callback_data="hiding")
        #         ]])
        #     )
        ...
        #     # Edit the original message with the final links
        #     stream_buttons = await query.message.edit_reply_markup(
        #         reply_markup=InlineKeyboardMarkup([
        #             [
        #                 InlineKeyboardButton("🛜 Download Link 🛜", url=dreamx_download),
        ...
        #     # Schedule deletion of the message with the links
        #     await asyncio.sleep(DELETE_TIME)
        #     await stream_buttons.delete()
        #     return
        ...
        #     print(e)
        #     await query.answer(f"⚠️ SOMETHING WENT WRONG: {e}", show_alert=True)
        #     # Restore the original buttons on error
        #     await query.message.edit_reply_markup(query.message.reply_markup)
        #     return
    ...
    #         # First, acknowledge the button click to stop the loading animation
    #         await query.answer()

    #         # Define the new, detailed alert message
    #         alert_text = (
    #             f"🚫 **𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱, {query.from_user.mention}!**\n\n"
    #             "👋 ᴛʜᴇ ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ / ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ ꜰᴇᴀᴛᴜʀᴇ ɪꜱ ᴇxᴄʟᴜꜱɪᴠᴇʟʏ ᴀᴠᴀɪʟᴀʙʟᴇ ꜰᴏʀ ᴏᴜʀ ✨ **ʜᴇʟʟ ᴋɪɴɢ ᴘʀᴇᴍɪᴜᴍ ᴍᴇᴍʙᴇʀꜱ** ✨.\n\n"
    #             "💎 **ᴜᴘɢʀᴀᴅᴇ ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ ᴛᴏ ᴜɴʟᴏᴄᴋ:**\n"
    #             "  • ⚡️ ɪɴꜱᴛᴀɴᴛ ꜱᴛʀᴇᴀᴍɪɴɢ ᴀᴄᴄᴇꜱꜱ\n"
    #             "  • 🛜 ᴜɴʟɪᴍɪᴛᴇᴅ ᴅᴀɪʟʏ ᴅᴏᴡɴʟᴏᴀᴅꜱ\n"
    #             "  • 🚫 100% ᴀᴅ-ꜰʀᴇᴇ ᴇxᴘᴇʀɪᴇɴᴄᴇ\n\n"
    #             "💰 **ᴠɪᴇᴡ ᴘʟᴀɴꜱ:** ᴜꜱᴇ ᴛʜᴇ `/premium` ᴏʀ `/plan` ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴇxᴘʟᴏʀᴇ ᴏᴜʀ ᴀꜰꜰᴏʀᴅᴀʙʟᴇ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ ᴏᴘᴛɪᴏɴꜱ.\n\n"
    #             "━━━━━━━━━━━━━━━\n"
    #             "🔥 ᴘᴏᴡᴇʀᴇᴅ ʙʏ: **𝐓𝐡𝐞 𝐇𝐞𝐥𝐥 𝐊𝐢𝐧𝐠**"
    #         )

    #         # Send the alert as a temporary reply message for better visibility
    #         try:
    #             temp_alert_msg = await query.message.reply_text(
    #                 text=alert_text,
    #                 quote=True,
    ...
    #             await temp_alert_msg.delete()
    #         except Exception as e:
    #             logger.error(f"Error sending/deleting 'prestream' alert: {e}")
...
# REPLACE the existing 'generate_stream_link' block in pmfilter.py with this one

    elif DreamxData.startswith("generate_stream_link"):
        # --- This is the new, centralized permission check ---
        user_id = query.from_user.id
        bot_id = client.me.id

        is_for_all = await db.watch_for_all_status(bot_id)
        user_is_allowed = is_for_all or user_id in ADMINS or await db.has_premium_access(user_id)

        if not user_is_allowed:
            await query.answer() # Acknowledge click
            alert_text = (
                f"🚫 **𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱, {query.from_user.mention}!**\n\n"
                "👋 ᴛʜᴇ ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ / ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ ꜰᴇᴀᴛᴜʀᴇ ɪꜱ ᴄᴜʀʀᴇɴᴛʟʏ ᴀᴠᴀɪʟᴀʙʟᴇ ᴏɴʟʏ ꜰᴏʀ ᴏᴜʀ ✨ **ʜᴇʟʟ ᴋɪɴɢ ᴘʀᴇᴍɪᴜᴍ ᴍᴇᴍʙᴇʀꜱ** ✨.\n\n"
                "💰 **ᴠɪᴇᴡ ᴘʟᴀɴꜱ:** ᴜꜱᴇ ᴛʜᴇ `/premium` ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴇxᴘʟᴏʀᴇ ᴏᴜʀ ᴀꜰꜰᴏʀᴅᴀʙʟᴇ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ ᴏᴘᴛɪᴏɴꜱ."
            )
            try:
                temp_alert_msg = await query.message.reply_text(
                    text=alert_text,
        ...
        # --- Original logic for users who pass the check ---
        _, file_id = DreamxData.split(":")
        try:
            username = query.from_user.mention
            generating_msg = await query.message.edit_reply_markup(
            ...
            stream_buttons = await query.message.edit_reply_markup(
                reply_markup=InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton("🛜 Download Link 🛜", url=dreamx_download),
                        InlineKeyboardButton('🖥️ Watch Online 🖥️', url=stream_stream)
                    ],
                    [
                        InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀтᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=UPDATE_CHNL_LNK)
                    ]
                ])
            )
        ...
        buttons = [[
            InlineKeyboardButton('𝐓𝐡𝐞 𝐇𝐞𝐥𝐥 𝐊𝐢𝐧𝐠 📜', url='https://t.me/the_hell_king_updates'),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
    ...
    try:
        if not spoll:
            ...
            if len(message.text) < 100:
                ...
                stick_id = "CAACAgUAAxkBAAIMimkE-VUQ7OwW1rjUXOvC1b5zuYRnAAIBAAPBJDExTOWVairA1m8eBA"
                # stick_id = ('').split()
#                 sticker_ids = [
#                     'CAACAgUAAxkBAAIMP2kE8-rThwJe5ctA_BKYTipkI7MtAAKzCQACcLRhV_Ivavr8fTGuHgQ',
#                     'CAACAgUAAxkBAAIMPmkE8-lKd-dxPwUIXms3UZ4nkrRPAAKzCQACcLRhV_Ivavr8fTGuHgQ',
#                     'CAACAgUAAxkBAAIMPWkE8-iq-BCffEd_iprRIlvTDf5mAAJ8CQAC7xtZV77nHFeEsNDzHgQ',
#                     'CAACAgUAAxkBAAIMPWkE8-iq-BCffEd_iprRIlvTDf5mAAJ8CQAC7xtZV77nHFeEsNDzHgQ',
#                     'CAACAgUAAxkBAAIMPGkE8-jyA6EBLA3P96U_lX6pkAsBAAJCCwACFzpZV3aNM96jv2J0HgQ',
#                     'CAACAgUAAxkBAAIMO2kE8-eGqXMb85kQFdO6yfrw_qdOAAKjCAACCtZgV9AzrCJD8zJsHgQ',
#                     'CAACAgUAAxkBAAIMOmkE8-bDZZfQ87k3dyF12YNnkjbtAAInCgACpERYV1h9DQZKbgABYh4E',
#                     'CAACAgUAAxkBAAIMOWkE8-ahLK4iSSnFSsPM8jR0yX32AAJuCgACtYthV-uSts6LgOzcHgQ',
#                     'CAACAgUAAxkBAAIMOGkE8-ULkAF1L7iJZH-KhAYHjPdEAAKlCwACms9YVyBeWwyBR2GiHgQ',
#                     'CAACAgUAAxkBAAIMN2kE8-RfziGFKcFfKFhzFC5EZ78GAAJ_CQACd_dZV8CemyL23-zbHgQ',
#                     'CAACAgUAAxkBAAIMNmkE8-O_XmO-y12hKKrpWp_yEalWAAJqCQACi9NgV3cNuFLQcNYNHgQ',
#                     'CAACAgUAAxkBAAIMNWkE8-JcSQsT-5igoUWOHDoAATRvHQAClQgAAvdEYFeHzmPWdl6k4R4E',
#                     'CAACAgUAAxkBAAIMNGkE8-HJbZSkeVzx8oDwP4eZrWYwAAKhCQACaVhhV-CsrlIOYgmiHgQ',
#                     'CAACAgUAAxkBAAIMM2kE8-CqQfb24Sl9-3f8sThT689ZAAIyCwACY4FZV6LkdvuFpJSTHgQ',
#                     'CAACAgUAAxkBAAIMMmkE8-Cww0vP6u6T3pGTnn-rke1wAAKBCwACTtNZV07LU3i9pN8sHgQ',
#                     'CAACAgUAAxkBAAIMMWkE89-Bv9bdBARGyvl-wmhWdg4zAALmCQACKqhgV5XcloaVA-4qHgQ',
#                     'CAACAgUAAxkBAAIMMGkE896VIwrgvS-eNg2_T5-E-jhPAAIKCwAC5p9gV9HUKuW13NpzHgQ',
#                     'CAACAgUAAxkBAAIMLWkE892F2geLXifNPja0sRGJj6nxAALZCQACM-xYVzOsUkjnEjsNHgQ',
#                     'CAACAgUAAxkBAAIMLGkE89yQoFS-w_0b3BpTzugxjnz0AAL_CAAC3PdhV3ZvpeP0FY3bHgQ',
#                     'CAACAgUAAxkBAAIMK2kE89uZGt2FiEqLJvMLEMBBhlNqAALYCgACNuthV465dnt12JerHgQ',
#                     'CAACAgUAAxkBAAIMKmkE89sX2Nc-9cjV9flsconXmEW8AAKlCQACZ2BYVzx82XiSVqi4HgQ',
#                     'CAACAgUAAxkBAAIMKWkE89p6sRtd0f8SSavy2NS-i6SHAAJDCwACnW5ZV20AAQ3jx6-VuR4E',
#                     'CAACAgUAAxkBAAIMJ2kE89nn-bEgHH2Q8jV8hEc0jYkCAALdCgACiVdhVxmB0t2QzTKsHgQ',
#                     'CAACAgUAAxkBAAIMJmkE88dA3n07zprzNZXg2y4X9-TYAALZCQACM-xYVzOsUkjnEjsNHgQ',
#                     'CAACAgUAAxkBAAIMJGkE88LZp3AO1s1JkyxJqphVVCKsAALYCgACNuthV465dnt12JerHgQ',
#                     'CAACAgUAAxkBAAIMI2kE88D4j6imYDDxGqfLplQnsrs_AAKlCQACZ2BYVzx82XiSVqi4HgQ',
#                     'CAACAgUAAxkBAAIMImkE877gMYclHs3PbmQKLs8MGoD0AAJDCwACnW5ZV20AAQ3jx6-VuR4E',
#                     'CAACAgUAAxkBAAIMIWkE87xCg1QqMPsUnukFOZ7JYaLBAAJ0CQAC8IlYV-WtjPzJbmmPHgQ'
# ]
                # stick_id =random.choice(sticker_ids)
        ...
        if imdb:
            ...
            if ULTRA_FAST_MODE:
                if settings.get('button'):
                    cap = f"<b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n⏰ ʀᴇsᴜʟᴛ ɪɴ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n⚜️ ᴘᴏᴡᴇʀᴇᴅ ʙʏ : ⚡ {message.chat.title or temp.B_LINK or '𝐓𝐡𝐞 𝐇𝐞𝐥𝐥 𝐊𝐢𝐧𝐠'} \n\n<u>Your Requested Files Are Here</u> \n\n</b>"
                else:
                    cap = f"<b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n⏰ ʀᴇsᴜʟᴛ ɪɴ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n⚜️ ᴘᴏᴡᴇʀᴇᴅ ʙʏ : ⚡ {message.chat.title or temp.B_LINK or '𝐓𝐡𝐞 𝐇𝐞𝐥𝐥 𝐊𝐢𝐧𝐠'} \n\n<u>Your Requested Files Are Here</u> \n\n</b>"
                    for idx, file in enumerate(files, start=1):
                        cap += f"<b>\n{idx}. <a href='https://telegram.me/{temp.U_NAME}?start=file_{message.chat.id}_{file.file_id}'>[{get_size(file.file_size)}] {clean_filename(file.file_name)}\n</a></b>"
...
#         temp.IMDB_CAP[message.from_user.id] = cap
#         if not settings["button"]:
#             cap+="<blockquote><b>😮‍💨 <u>Your Requested Files</u> 👇\n</b></blockquote>"
#             for file in files:
...
#     else:
#         if settings["button"]:
#             cap = f"<blockquote><b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n📂 ᴛᴏᴛᴀʟ ꜰɪʟᴇꜱ : <code>{total_results}</code>\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n⏰ ʀᴇsᴜʟᴛ ɪɴ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n😮‍💨 𝑹𝒆𝒒𝒖𝒆𝒔𝒕𝒆𝒅 𝑭𝒊𝒍𝒆𝒔 👇 \n\n</b><blockquote>"
#         else:
#             cap = f"<blockquote><b>🏷 ᴛɪᴛʟᴇ : <code>{search}</code>\n📂 ᴛᴏᴛᴀʟ ꜰɪʟᴇꜱ : <code>{total_results}</code>\n📝 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {message.from_user.mention}\n⏰ ʀᴇsᴜʟᴛ ɪɴ : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n\n😮‍💨 𝑹𝒆𝒒𝒖𝒆𝒔𝒕𝒆𝒅 𝑭𝒊𝒍𝒆𝒔 👇 \n\n</b><blockquote>"
