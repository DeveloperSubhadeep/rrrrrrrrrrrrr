...
from dreamxbotz.util.render_template import render_page, render_download_page
...
@routes.get(r"/download/{id:\d+}/{name:\S+}", allow_head=True)
async def download_page_handler(request: web.Request):
    try:
        id = int(request.match_info["id"])
        secure_hash = request.rel_url.query.get("hash")

        # Call the new download page renderer
        html_content = await render_download_page(id, secure_hash)
        return web.Response(text=html_content, content_type='text/html')
        
    except InvalidHash:
        raise web.HTTPForbidden(text="Invalid Link: Hash is invalid.")
    except FIleNotFound:
        raise web.HTTPNotFound(text="File not found on the server.")
    except Exception as e:
